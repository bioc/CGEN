...
mySnp.logistic2=function(y,x1,x2,covs,x.st,strDat,control,g.model="dom",optim.method="BFGS",
    followup=F,bNames=c("Intercept","x1","x2","x1:x2"),doNull=F, use.C.code=1,
     genetic.model=genetic.model, snp.new=NULL){    
    ...
    #this runs snp.logistic after constructing appropriate data
      ...
      if(is.null(covs)==F){
          ...
          if(indic.st==T) myDATA3=data.frame(y=y,x1=x1,x2=as.factor(x2),covs,stVar=as.factor(x.st))
          if(indic.st==F) myDATA3=data.frame(y=y,x1=x1,x2=as.factor(x2),covs)

      }#end of

      if(is.null(covs)==T){

          if(indic.st==T) myDATA3=data.frame(y=y,x1=x1,x2=as.factor(x2),stVar=as.factor(x.st))
          if(indic.st==F) myDATA3=data.frame(y=y,x1=x1,x2=as.factor(x2))
      ...
      if (doNull==T) {
         ...
         if(indic.st==T) lm2=snp.logistic(myDATA3,"y","x1",main.vars=c("x2",vars0),op=op,strata.var="stVar")
         if(indic.st==F) lm2=snp.logistic(myDATA3,"y","x1",main.vars=c("x2",vars0),op=op)             
           
         indic.ok = "CML" %in% names(lm2) # then CML was successfuly calculated
         if(indic.ok==F){    stop("CML failed to be optimized by snp.logistic") }  
         lenux2 <- length(unique(x2)) - 1
         if (lenux2) op$init.parms <- c(lm2$CML$parms, rep(0.0, nsnpcats*lenux2), lm2$CML$strata.parms)         
      }#end o 

      if (indic.st==T) lm0=snp.logistic(myDATA3,"y","x1",main.vars=c("x2",vars0),int.vars=c("x2"),op=op,strata.var="stVar")
      if (indic.st==F) lm0=snp.logistic(myDATA3,"y","x1",main.vars=c("x2",vars0),int.vars=c("x2"),op=op)

      indic.ok = "CML" %in% names(lm0) # then CML was successfuly calculated
     
      if(indic.ok==F){    print("CML failed to be optimized by snp.logistic") }
      ...
      if(indic.ok==T){ 
            ...
            ############### [4] Fit the model using optim() to report the data for null model fitting ##################
             ...
             if(followup==T){  ### i am only doing this for dominant model didn't chekc g.model="general" yet
            ...
            }#end of if(followup==T){  ### i am only doing this for dominant model didn't chekc g.model="general" yet
