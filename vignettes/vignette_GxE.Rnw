...
\section*{Comparison with snp.scan.logistic}
Both GxE.scan and snp.scan.logistic can be used for conducting a GWAS scan, however GxE.scan has additional features.
GxE.scan can process a GWAS scan using the snp.logistic, additive.test, score.test or snp.matched functions, 
whereas snp.scan.logistic only calls snp.logistic.  
GxE.scan can process more genotype formats by calling PLINK or GLU to automatically transform the genotype data,
but in snp.scan.logistic the user would have to manually transform the data beforehand.
GxE.scan streams the genotype data by reading in a single SNP at a time so that there will be no memory issues, but 
snp.logistic reads in a set of SNPs all at once so that memory could run out if too many SNPs are read.
GxE.scan also has the capacity for users to run their own customized scan by writing a simple R function; a
capability not possible with snp.scan.logistic.  
...
\section*{Example of GxE.scan with an imputed genotype file}

Assume that we have an imputed genotype file from the IMPUTE2 software and a list of ids specifying
the order of the subjects in the genotype file.

<<Impute files>>=
geno.file <- system.file("sampleData", "geno_imputed.txt.gz", package="CGEN")
subject.file <- system.file("sampleData", "subjects_imputed.txt", package="CGEN")
geno.file
subject.file
@

The file subject.file contains a single column of subject ids without a variable header. 
The subject.list option can be defined either way below. 
<<Impute subject.list>>=
subject.list <- list(file=subject.file, id.var=-1, header=0)
subject.list <- subject.file
@

Define the snp.list argument. The genotype file corresponds to format="impute" or file.type=10.
<<snp.list>>=
snp.list <- list(file=geno.file, format="impute", subject.list=subject.list)
@

Define the pheno.list argument. The column "Imputed.ID" in pheno.file has ids matching the ids in subject.file.
<<Impute pheno.list>>=
pheno.list <- list(file=pheno.file, id.var="Imputed.ID", file.type=3, delimiter="\t", 
              response.var="CaseControl", int.vars="Exposure",
              main.vars=c("Gender", "Exposure", "Smoke_CURRENT", "Smoke_FORMER"))
@
              
Let us also use the subsetData option of pheno.list to run the analysis on the subset
of subjects from study A.
<<subset data>>=
temp <- list(var="Study", operator="%in%", value="A")
pheno.list$subsetData <- list(temp)
@
...
contains the SNP and will be coded as 0-1-2 or NA for directly genotyped files. 
However if the genotype file was imputed, then the SNP variable would be the
expected genotype Prob(G=1) + 2*Prob(G=2), and there would be 3 additional columns in the data frame
called "SNP.ProbG0", "SNP.ProbG1" and "SNP.ProbG2" corresponding to the imputed probabilities Prob(G=0),
Prob(G=1) and Prob(G=2).
...
myscan <- function(data, lists) {
 ...
 se   <- temp$cov["SNP:Exposure", "SNP:Exposure"] 
