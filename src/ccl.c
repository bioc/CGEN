...
enum {CONTROL, CASE} Status ;

typedef struct RDATA
	{
		int nsub, p_snp, p_main, p_int, numSZ, mnsz, *D, *osx, *usz, *mdx, *nsz, *cnsz, *ndx ; 
...
void ccl_optim(double *beta, int *maxit, double *tol, int *xnsub, int *D, double *x_snp, int *xp_snp, double *x_main, int *xp_main,
...
static double ccl_grad_hess(double *beta, double *grad, double **hess, LSUM *lsum, CDAT *cdat, tnode *root, double loglike1, double *grad1, Rdata *rda) ;
...
/* Function to maximize CCL likelihood using Newton-Raphson. Should be called from R */
void ccl_optim(double *beta, int *maxit, double *tol, int *xnsub, int *D, double *x_snp, int *xp_snp, double *x_main, int *xp_main,
			   double *x_int, int *xp_int, int *xnumSZ, int *xmnsz, int *usz, int *mdx, int *nsz, int *cnsz, int *osx, int *ndx, double *LOGLIKE, double *HESS, 
			   int *CONV, int *ITER)
{
	...
	{
		loglike = ccl_grad_hess(beta, grad, hess, lsum, cdat, root, loglike1, grad1, rda) ;
...
/* Function to compute the gradient and hessian of CCL, for a specified 
 parameter vector beta. Returns Log-likelihood. */
double ccl_grad_hess(double *beta, double *grad, double **hess, LSUM *lsum, CDAT *cdat, tnode *root, double loglike1, double *grad1, Rdata *rda)
{
	int i, j, k, u , t_offset, p = rda -> p_snp + rda -> p_main + rda -> p_snp * rda -> p_int, S = rda -> usz[rda -> numSZ - 1] ;
	double lglk  ;
	tnode *uroot ;
	...
	for(u = 0 ; u < rda -> numSZ ; u++)
	{
		...
		if(DFACT) cdat -> dfact_current = factorial(rda -> usz[u]) ;
		else cdat -> dfact_current = 1 ;
		/* tree_print(uroot, 1) ; */
		...
		for(i = 0 ; i < rda -> nsz[u] ; i++)
		{
			//if(rda -> ndx[rda -> cnsz[u] + i * rda -> usz[u]] == rda -> usz[u]) continue ;
	...
	return(lglk) ;
} /* End of ccl_grad_hess() */


/* Function to traverse the tree and calculate L, LX and LXX for all matched sets of a fixed size */
void tree_traverse(tnode *node, int u, int t_offset, double *beta, CDAT *cdat, LSUM *lsum, Rdata *rda, int is_root)
{
	register int i, gix, hix, j1, j2 , p = rda -> p_snp + rda -> p_main + rda -> p_snp * rda -> p_int ;
	register double ll, dtmp ;
	
	if(CLR && is_root == 0 && node -> g != node -> h)
	{
	...
	if(is_root == 1)
	{
		for(i = 0 ; i < rda -> nsz[u] ; i++) { if(rda -> ndx[rda -> cnsz[u] + i * rda -> usz[u]] == 0) lsum -> L[i] = cdat -> dfact_current ; }
	}
	else
	{
		for(i = 0 ; i < rda -> nsz[u] ; i++)
		{
			...
			{
				cdat -> X_current[rda -> p_snp + j2][i] += ELEM(rda -> x_main , gix , j2 , rda -> nsub) ;
				ll += (beta[rda -> p_snp + j2] * cdat -> X_current[rda -> p_snp + j2][i]) ; /* b'X */
			}
			for(j1 = 0 ; j1 < rda -> p_snp ; j1++)
			{
				cdat -> X_current[j1][i] += ELEM(rda -> x_snp , hix , j1 , rda -> nsub) ;
				ll += (beta[j1] * cdat -> X_current[j1][i]) ;
				for(j2 = 0 ; j2 <  rda -> p_int ; j2++)
				{
					dtmp = ELEM(rda -> x_snp , hix , j1 , rda -> nsub) * ELEM(rda -> x_int , gix , j2 , rda -> nsub) ;
					cdat -> X_current[rda -> p_snp + rda -> p_main + j1 * rda -> p_int + j2][i] += dtmp ;
					ll += (beta[rda -> p_snp + rda -> p_main + j1 * rda -> p_int + j2] * cdat -> X_current[rda -> p_snp + rda -> p_main + j1 * rda -> p_int + j2][i]) ;
				}
			}
			if((rda -> usz[u] - cdat -> d_current) == (rda -> ndx[rda -> cnsz[u] + i * rda -> usz[u]]))
			{
				/* exp(b'X) */
				ll = exp(ll) ;
				/* L += exp(b'X) */
				lsum -> L[i] += (ll * cdat -> dfact_current) ;
				for(j1 = 0 ; j1 < p ; j1 ++)
				{
					lsum -> LX[j1][i] += (ll * cdat -> dfact_current * cdat -> X_current[j1][i]) ;	/* LX += exp(b'X) X */
					for(j2 = 0 ; j2 <= j1 ; j2++) 
					{
						/* LXX += exp(b'X) XX' */
						lsum -> LXX[j1][j2][i] += (ll * cdat -> dfact_current * cdat -> X_current[j1][i] * cdat -> X_current[j2][i]) ;
	...
	if((node -> child != NULL) && (rda -> usz[u] - cdat -> d_current) < rda -> mdx[u])
	{
		/* Update depth before traversing children */
		if(DFACT) cdat -> dfact_current /= cdat -> d_current ;
		cdat -> d_current -= 1 ;
	...
	if(is_root == 0)
	{
		...
		{			
			...
			for(j2 = 0 ; j2 < rda -> p_main ; j2++) cdat -> X_current[rda -> p_snp + j2][i] 
				-= ELEM(rda -> x_main , gix , j2 , rda -> nsub) ;
			for(j1 = 0 ; j1 < rda -> p_snp ; j1++)
			{
				cdat -> X_current[j1][i] -= ELEM(rda -> x_snp, hix , j1 , rda -> nsub)  ;
				for(j2 = 0 ; j2 <  rda -> p_int ; j2++)
					cdat -> X_current[rda -> p_snp + rda -> p_main + j1 * rda -> p_int + j2][i] 
...
double numr_grad(double *gr, int start, int len, double *beta, Rdata *rda)
{
	register int i, j1, j2 , p = rda -> p_snp + rda -> p_main + rda -> p_snp * rda -> p_int ;
	double ll = 0 , dtmp ;
	...
	for(i = start ; i < (start + len) ; i++)
	{
		...
		for(j2 = 0 ; j2 < rda -> p_main ; j2++)
		{
			gr[rda -> p_snp + j2] += ELEM(rda -> x_main , rda -> osx[i] , j2 , rda -> nsub) ;	/*  X  */
			ll += (beta[rda -> p_snp + j2] *  ELEM(rda -> x_main , rda -> osx[i] , j2 , rda -> nsub)) ;  /* X'b  */
		}
		for(j1 = 0 ; j1 <  rda -> p_snp ; j1++)
		{
			gr[j1] += ELEM(rda -> x_snp , rda -> osx[i] , j1 , rda -> nsub) ;
			ll += (beta[j1] *  ELEM(rda -> x_snp , rda -> osx[i] , j1 , rda -> nsub)) ;
			for(j2 = 0 ; j2 <  rda -> p_int ; j2++)
			{
				dtmp = ELEM(rda -> x_snp, rda -> osx[i] , j1 , rda -> nsub) * ELEM(rda -> x_int , rda -> osx[i] , j2 , rda -> nsub) ;
				gr[rda -> p_snp + rda -> p_main + j1 * rda -> p_int + j2] += dtmp ;
				ll += (beta[rda -> p_snp + rda -> p_main + j1 * rda -> p_int + j2] * dtmp) ;
			}
		}		
	}
	return(ll) ;	
...
double rel_error(double *beta, double *delta, int p)
{
	register int j ;
